...
impl CursorManager {
    pub fn new(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let last_processed_block = match fs::read_to_string(&file_path) {
            Ok(contents) => {
                let block_num = contents.trim().parse::<u64>()
                    .wrap_err_with(|| format!("failed to parse cursor file as u64: {:?}", file_path))?;
            ...
            Err(e) => {
                return Err(e).wrap_err_with(|| format!("failed to read cursor file: {:?}", file_path));
            }
        };
...
mod tests {
    ...
    fn test_cursor_manager_update_and_reload() {
        ...
        assert_eq!(cursor2.last_processed_block, 100, "cursor should persist across restarts");
    ...
    fn test_cursor_manager_multiple_updates() {
        ...
        for block_num in blocks {
            cursor.update_cursor(block_num).expect("failed to update cursor");
    ...
    fn test_cursor_atomic_write() {
        ...
        assert!(!temp_path.exists(), "temporary file should not exist after update");
