...
impl SchemaManager {
    ...
    pub async fn initialize_schema(&self) -> Result<()> {
        ...
        for (idx, statement) in schema_sql.split(';').enumerate() {
            ...
            self.client
                ...
                .await
                .wrap_err_with(|| {
                    format!("failed to execute schema statement {}: {}", idx, statement)
                })?;
        }
    ...
    pub async fn verify_schema(&self) -> Result<()> {
        ...
        for table in required_tables {
            let result: Result<u8, clickhouse::error::Error> = self
                .client
            ...
            match result {
                ...
                Ok(val) => {
                    return Err(eyre::eyre!(
                        "unexpected value {} when checking table '{}'",
                        val,
                        table
                    ))
                }
...
mod tests {
    ...
    fn spawn_clickhouse(
        docker: &Cli,
    ) -> (testcontainers::Container<'_, GenericImage>, u16, String) {
    ...
    async fn test_clickhouse_connection() {
        ...
        assert!(
            result.is_ok(),
            "should successfully execute SELECT 1: {:?}",
            result.err()
        );
    ...
    async fn test_clickhouse_version() {
        ...
        assert!(
            result.is_ok(),
            "should successfully get version: {:?}",
            result.err()
        );
    ...
    async fn test_schema_initialization() {
        ...
        assert!(
            verify_result.is_ok(),
            "schema verification should succeed: {:?}",
            verify_result.err()
        );
    ...
    async fn test_table_structure() {
        ...
        let engine_result: Result<String, clickhouse::error::Error> = client
            .query(
                "SELECT engine FROM system.tables WHERE database = 'default' AND name = 'blocks'",
            )
            .fetch_one()
            .await;
        ...
        assert!(
            engine.contains("CollapsingMergeTree"),
            "engine should be CollapsingMergeTree, got: {}",
            engine
        );
